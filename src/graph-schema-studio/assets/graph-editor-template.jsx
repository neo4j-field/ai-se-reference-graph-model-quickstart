  ...
  if (selected.type === "node") {
    ...
    return (
      <div style={{ padding: 16 }}>
        ...
        <span style={labelStyle}>Caption (primary label)</span>
        <input
          value={node.caption}
          onChange={e => {
...
function ImportModal({ onImport, onClose }) {
  ...
  const handleImport = () => {
    try {
      ...
      onImport({
        nodes: g.nodes.map(n => {
          const caption = n.caption || (n.labels?.[0]) || "";
          // Enforce the invariant: labels = [caption, ...rest], no dupes,
          // no empties. This matches what inject.py does server-side so the
          // editor behaves consistently whether a graph is injected or pasted.
          const seen = new Set();
          const extras = [];
          ...
          return {
            ...
            style: { color: n.style?.color || COLORS[Math.floor(Math.random() * COLORS.length)], radius: n.style?.radius || 50 },
          };
        }),
